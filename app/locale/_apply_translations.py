...
TRANSLATIONS = {
    ...
    # --- editor blocks/sectors (GrapesJS UI) ---
    "Structure": ("Structure", "Structuur"),
    "Basic": ("Basique", "Basis"),
    "2 columns": ("2 colonnes", "2 kolommen"),
    "3 columns": ("3 colonnes", "3 kolommen"),
    "Heading": ("Titre", "Kop"),
    "Text": ("Texte", "Tekst"),
    "Your text here.": ("Votre texte ici.", "Jouw tekst hier."),
    "Image": ("Image", "Afbeelding"),
    "Button": ("Bouton", "Knop"),
    "Click here": ("Cliquez ici", "Klik hier"),
    "Divider": ("Séparateur", "Scheidingslijn"),
    "Dimensions": ("Dimensions", "Afmetingen"),
    "Typography": ("Typographie", "Typografie"),
    "Decorations": ("Décorations", "Decoratie"),
    "Extra": ("Extra", "Extra"),
    ...
    'Fee rule': ("Règle de cotisation", "Lidgeldregel"),
    'Fee rules': ("Règles de cotisation", "Lidgeldregels"),
    '1st member': ("1er membre", "1ste lid"),
    '2nd member': ("2e membre", "2de lid"),
    '3rd member or more': ("3e membre ou plus", "3de lid of meer"),
    'Leave blank to apply to all branches': ("Laisser vide pour appliquer à toutes les branches", "Laat leeg om toe te passen op alle takken"),
    'Prices': ("Tarifs", "Prijzen"),
    'Branch': ("Branche", "Tak"),
    'All branches': ("Toutes les branches", "Alle takken"),
    'Animators': ("Animateurs", "Animators"),
    'No price defined.': ("Aucun tarif défini.", "Geen prijs gedefinieerd."),
    'Edit prices': ("Modifier les tarifs", "Prijzen bewerken"),
    'Prices saved for %(range)s.': ("Tarifs enregistrés pour %(range)s.", "Prijzen opgeslagen voor %(range)s."),
    'Leave a field blank to remove the price.': ("Laissez un champ vide pour supprimer le tarif.", "Laat een veld leeg om de prijs te verwijderen."),
    ...
    # --- attestations ---
    "One page per document": ("Un document par page", "Eén document per pagina"),
    "New document on a marker page": ("Nouveau document sur une page repère", "Nieuw document op een markeringspagina"),
    "First page only": ("Première page uniquement", "Alleen de eerste pagina"),
    "Every page": ("Chaque page", "Elke pagina"),
    "Draft": ("Brouillon", "Concept"),
    "Configuring": ("Configuration", "Configureren"),
    "Ready": ("Prêt", "Klaar"),
    "Attestation campaign": ("Campagne d'attestations", "Attestatiecampagne"),
    "Attestation campaigns": ("Campagnes d'attestations", "Attestatiecampagnes"),
    'With marker mode, a new document starts on a page whose text begins with this marker (e.g. \\"Aux parents de\\").': (
        "En mode repère, un nouveau document commence sur une page dont le texte commence par ce repère (p. ex. « Aux parents de »).",
        "In markeringsmodus begint een nieuw document op een pagina waarvan de tekst met deze markering begint (bv. \"Aux parents de\").",
    ),
    "Attestation item": ("Élément d'attestation", "Attestatie-item"),
    "Attestation items": ("Éléments d'attestation", "Attestatie-items"),
    "Skipped": ("Ignoré", "Overgeslagen"),
    "Name as shown on the first page": ("Nom tel qu'affiché sur la première page", "Naam zoals getoond op de eerste pagina"),
    'Type exactly what you see (e.g. \\"Dupont Jean\\").': (
        "Tapez exactement ce que vous voyez (p. ex. « Dupont Jean »).",
        "Typ exact wat u ziet (bv. \"Dupont Jean\").",
    ),
    "Address as shown on the first page (optional)": ("Adresse telle qu'affichée sur la première page (facultatif)", "Adres zoals getoond op de eerste pagina (optioneel)"),
    "Used to disambiguate people who share a name.": ("Utilisé pour distinguer les personnes portant le même nom.", "Wordt gebruikt om personen met dezelfde naam te onderscheiden."),
    "Signature offset X (points)": ("Décalage de signature X (points)", "Handtekeningoffset X (punten)"),
    "Signature offset Y (points)": ("Décalage de signature Y (points)", "Handtekeningoffset Y (punten)"),
    "Attestation for {prenom} {nom}": ("Attestation pour {prenom} {nom}", "Attestatie voor {prenom} {nom}"),
    "Use {prenom} and {nom} as variables.": ("Utilisez {prenom} et {nom} comme variables.", "Gebruik {prenom} en {nom} als variabelen."),
    'Dear parent,\\n\\nPlease find attached the attestation for {prenom} {nom}.\\n\\nBest regards,\\nThe unit staff': (
        "Cher parent,\n\nVeuillez trouver en pièce jointe l'attestation pour {prenom} {nom}.\n\nBien à vous,\nLe staff d'unité",
        "Beste ouder,\n\nIn bijlage vindt u de attestatie voor {prenom} {nom}.\n\nMet vriendelijke groeten,\nDe eenheidsstaf",
    ),
    "Could not find that value on the first page.": ("Impossible de trouver cette valeur sur la première page.", "Deze waarde kon niet gevonden worden op de eerste pagina."),
    "%(sent)s sent, %(failed)s failed.": ("%(sent)s envoyé(s), %(failed)s échec(s).", "%(sent)s verzonden, %(failed)s mislukt."),
    "Attestations": ("Attestations", "Attestaties"),
    "Upload, sign and email attestation PDFs.": ("Téléversez, signez et envoyez les PDF d'attestations.", "Upload, onderteken en e-mail attestatie-PDF's."),
    "New campaign": ("Nouvelle campagne", "Nieuwe campagne"),
    "Items": ("Éléments", "Items"),
    "Created": ("Créé", "Aangemaakt"),
    "Review": ("Vérifier", "Controleren"),
    "No campaign yet.": ("Aucune campagne pour l'instant.", "Nog geen campagne."),
    "Configure": ("Configurer", "Configureren"),
    "New attestation campaign": ("Nouvelle campagne d'attestations", "Nieuwe attestatiecampagne"),
    "Upload the document PDF and the signature PDF.": ("Téléversez le PDF des documents et le PDF de signature.", "Upload de document-PDF en de handtekening-PDF."),
    "Configure attestation": ("Configurer l'attestation", "Attestatie configureren"),
    "Teach the app where the name lives and position the signature.": ("Indiquez à l'application où se trouve le nom et positionnez la signature.", "Geef aan waar de naam staat en plaats de handtekening."),
    "The signature PDF should be the same page size as the documents (e.g. A4), transparent except for the signature. Drag the signature over the page to position it.": (
        "Le PDF de signature doit avoir la même taille de page que les documents (p. ex. A4), transparent sauf pour la signature. Faites glisser la signature sur la page pour la positionner.",
        "De handtekening-PDF moet hetzelfde paginaformaat hebben als de documenten (bv. A4), transparant behalve de handtekening. Sleep de handtekening over de pagina om ze te plaatsen.",
    ),
    "First page": ("Première page", "Eerste pagina"),
    "Data to extract": ("Données à extraire", "Te extraheren gegevens"),
    "Preview recipients": ("Aperçu des destinataires", "Voorbeeld van ontvangers"),
    "Extracted text of the first page": ("Texte extrait de la première page", "Geëxtraheerde tekst van de eerste pagina"),
    "Review attestations": ("Vérifier les attestations", "Attestaties controleren"),
    "Check who each document goes to, then send.": ("Vérifiez le destinataire de chaque document, puis envoyez.", "Controleer wie elk document ontvangt en verzend."),
    "For each document, confirm the recipient or skip it. Recipients are re-resolved from the selected person when you send.": (
        "Pour chaque document, confirmez le destinataire ou ignorez-le. Les destinataires sont recalculés à partir de la personne sélectionnée à l'envoi.",
        "Bevestig voor elk document de ontvanger of sla over. Ontvangers worden bij verzending opnieuw bepaald op basis van de geselecteerde persoon.",
    ),
    "Pages": ("Pages", "Pagina's"),
    "Extracted name": ("Nom extrait", "Geëxtraheerde naam"),
    "Extracted address": ("Adresse extraite", "Geëxtraheerd adres"),
    "Will send to": ("Sera envoyé à", "Wordt verzonden naar"),
    ...
    "Send signed attestations": ("Envoyer les attestations signées", "Ondertekende attestaties verzenden"),
